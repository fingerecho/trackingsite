...
def create_charts():
    ...
    nodes = [{"label":"fuck1","name": "结点1", "symbolSize": 30},
             ...
             {"label":"fuck2","name": "结点8", "symbolSize": 30}]
    links = []
    index = 0
    drawd = []
    for i in nodes:
        for j in nodes:
            if index < 100 and j not in drawd:
                links.append({"source": i.get('name'), "symbol":"arrow","value":"hello","target": j.get('name')})
                index = index + 1
                drawd.append(i)
    ...
    chart.add("", nodes, links, label_pos="right", graph_repulsion=50,graph_edge_symbol='helloworldworld',is_legend_show=False, line_curve=0.1, label_text_color=None)
    page.add(chart)
