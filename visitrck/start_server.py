...
class MULP(Process):
    ...
    def run(self):
        init_(self.port)
def init_python_pro():
    #call(['source','activate','python36-for-web'])
    #flag_0 = not 1==call(['python','-m','compileall','.'])
...
#    
    ...
    if True:    
        ...
        return 0
def init_(port):
    if 1==call(['nohup','python','serv.py',port,'&']):
        print("success")
    else:
        print("failed")
    #sleep(1)
if __name__ == "__main__":
    if 0 == init_python_pro():
        exit()
    #call(['source','activate','python36-for-web'])
    st,ed = sys.argv[1],sys.argv[2]
    for port in range(int(st),int(ed)):
        tmp = MULP(port)
        tmp.start()
